...
const Dashboard = {
  ...
  /** 文本像素宽度估算 —— SVG 里我们是**拼字符串**，没有布局引擎可查。
   *  汉字/全角 ≈ 1em，数字/字母/半角标点 ≈ 0.55em。
   *  绝不能拿「字数」当宽度：中英混排时必然失准（见 fitLabel 的注释）。 */
  textW(str, fontSize) {
    let w = 0;
  ...
  /** 把标签截到不超过 maxPx（末尾补省略号），**从头部截**。
   *  历史 bug（用户报告「名称很长的显示不全，只能看到中间几个字」）：
   *    原先按「字数」截断（客户图 labelMax = 12 字 → 144px），而该图标签区
   *    可用宽度只有 122px，又用 text-anchor="end" 右对齐定位 → 文字向左溢出
   *    被 SVG 裁掉开头，屏幕上只剩中间一段（实测「山西普能控股集团某煤业有…」
   *    左溢 34px）。宽度必须按像素算，且截断必须发生在尾部。 */
  fitLabel(str, maxPx, fontSize) {
    const s = String(str);
    if (this.textW(s, fontSize) <= maxPx) return s;
    const ellW = fontSize;                       // 「…」按 1em 计（保守）
    let out = '';
    let w = 0;
    for (const ch of s) {
  ...
  /* ---------- 客户条形图（客户名普遍长，标签列上限放宽） ---------- */
  custChart(bars, W) {
    return this.barChart(bars, {
      color: C_PRIMARY, labelW: 132, maxLabelW: 200, title: '客户应收余额',
      empty: this.EMPTY_OWED,
    }, W);
  },
  ...
  attrChart(bars, W) {
    return this.barChart(bars, {
      labelW: 120, colorBy: name => groupColorOf(name), title: '客户属性欠款构成',
      empty: this.EMPTY_OWED,
    }, W);
  },
  ...
  barChart(bars, opts = {}, W = 520) {
    ...
    const color = opts.color || C_PRIMARY;
    const FS = 12;                       // 标签字号（须与下面 <text font-size> 一致）
    const base = opts.labelW || 96;
    /* 标签列宽自适应：按本图**最长名称**的估算宽度定，下限 base、上限
       min(opts.maxLabelW, 容器 38%)。写死一个值会让长名被截得过狠、短名白占空间。
       左侧用掉的宽度直接决定柱子可用宽 innerW，故上限必须有，否则柱子被挤没。
       +20 而非 +12：右边距已吃掉 12（见 fitLabel 调用），余量必须**大于**右边距，
       否则估算误差会让最长的那个名称差 0.2px 就被误截（实测过）。 */
    const cap = Math.max(base, Math.min(opts.maxLabelW || 180, Math.round(W * 0.38)));
    const nameNeed = Math.max(...list.map(b => this.textW(b[0], FS))) + 20;
    const labelW = Math.round(Math.min(Math.max(nameNeed, base), cap));
    ...
    const svg = list.map((b, i) => {
      ...
      const label = this.fitLabel(name, labelW - 12, FS);
      ...
      const tip = unfin > 0
        ...
        : `${name} ${this.wan(bal)}`;
      /* 标签**左对齐**（默认 text-anchor:start，x=0）：名称列左缘齐平，比右对齐
         贴着柱子参差不齐更整齐；截断只发生在尾部，读到的始终是名称开头。 */
      return `<g><title>${Utils.escapeHtml(tip)}</title>
        <text x="0" y="${y + barH / 2 + 4}" font-size="${FS}" fill="${C_INK_600}">${Utils.escapeHtml(label)}</text>
